...
class PPO:
    def __init__(self,
        ...
        # actor_critic_class <class 'algorithms.rl.ppo.module.ActorCritic'>
        self.actor_critic = actor_critic_class(self.observation_space.shape, self.state_space.shape, self.action_space.shape,
                                               init_noise_std, model_cfg, asymmetric=asymmetric, use_pc = self.is_vision, device=self.device)
    ...
    def save(self, path, iteration_num):
        checkpoint = {
        ...
        if self.is_testing:
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    # Step the vec_environment
                    # torch.clamp(self.task.obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device), self.task.rew_buf.to(self.rl_device), self.task.reset_buf.to(self.rl_device), self.task.extras
                    next_obs, rews, dones, infos = self.vec_env.step(actions, id)
            ...
            self.save(os.path.join(self.log_dir, 'model_{}.pt'.format(num_learning_iterations)), num_learning_iterations)

    def detach_info(self, info_dict):
        detached_info = {}
        for k, v in info_dict.items():
            if isinstance(v, torch.Tensor):
                detached_info[k] = v.detach()
            else:
                detached_info[k] = v
