...
class ShadowHandGrasp(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        # ==================================mult scale of object======================================
        # the all object used for train
        object_scale_dict = self.cfg['env']['object_code_dict']
        print('object_scale_dict num:',len(object_scale_dict))
        ...
        # 建立尺度和ID的双向映射
        self.id2scale = [] # 通过ID索引尺度（列表）
        self.scale2id = {} # 通过尺度查找ID（字典）
        ...
        self.grasp_data = {}
        for object_code in self.grasp_data_np:
            self.grasp_data[object_code] = {}
            for scale in self.grasp_data_np[object_code]:
                self.grasp_data[object_code][scale] = {key: None for key in keys_to_convert}
        # print('self.grasp_data_np',self.grasp_data_np)
        # print('self.grasp_data',self.grasp_data)
        ...
        table_texture_files = os.join(assets_path, table_texture_files)
        ...
        # print('self.object_code_list',self.object_code_list)
        # pdb.set_trace()
        for object_id in range(len(self.object_code_list)):
            ...
            for scale_id in self.object_scale_id_list[object_id]:
                ...
                # print('visual_feat_root',visual_feat_root)
        ...
        for object_id, object_code in enumerate(self.object_code_list):
            object_code_dictobject_asset_options = gymapi.AssetOptions()
            ...
            for obj_id, scale_id in object_scale_idx_pairs:
                if obj_id == object_id:
                    ...
                    print('scaled_object_asset_file',scaled_object_asset_file)
        ...
        # ===================================create table asset=====================================
        table_dims = gymapi.Vec3(1, 1, 0.6) # 桌子尺寸 (长宽高)
        asset_options = gymapi.AssetOptions()
    ...
    def _load_cameras(self, env_ptr, env_id, camera_props, camera_eye_list, camera_lookat_list):
        ...
        print('camera_props',camera_props)
        ...
        for i in range(self.num_cameras):
            ...
            print(1)
            if camera_handle == -1:
                raise RuntimeError(f"Failed to create camera in environment {env_id}")
...
def compute_hand_reward(
        object_init_z, delta_qpos, delta_target_hand_pos, delta_target_hand_rot,  id: int, object_id, dof_pos, rew_buf, reset_buf, reset_goal_buf, progress_buf, successes, current_successes, consecutive_successes,
        max_episode_length: float, 
        object_pos, object_handle_pos, object_back_pos, object_rot, target_pos, target_rot,
        ...
        success_tolerance: float, reach_goal_bonus: float, fall_dist: float, fall_penalty: float, max_consecutive_successes: int, av_factor: float, goal_cond: bool
):
    # Distance from the hand to the object
    # 目标位置与物体当前位置的欧式距离 (L2范数)
    goal_dist = torch.norm(target_pos - object_pos, p=2, dim=-1)
    # 右手到目标的距离
    # self.object_handle_pos = self.object_pos
    goal_hand_dist = torch.norm(target_pos - right_hand_pos, p=2, dim=-1)
    # 右手到物体把手的距离 (超过0.5则截断为0.5)
